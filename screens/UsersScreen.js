...
export const UsersScreen = () => {
  ...
  const [formData, setFormData] = useState({ name: '', email: '', contact: '', roles: ['Employee'] });
  ...
  const resetForm = () => {
    setEditingUser(null);
    setFormData({ name: '', email: '', contact: '', roles: ['Employee'] });
  };

  const openModal = (user = null) => {
    if (user) {
      setEditingUser(user);
      setFormData({ name: user.name, email: user.email, contact: user.contact || '', roles: user.roles || ['Employee'] });
    } else {
      resetForm();
    }
...
const styles = StyleSheet.create({
  ...
  roleChip: { backgroundColor: '#f0f0f0', padding: 8, borderRadius: 8, marginRight: 8, marginBottom: 8, alignSelf: 'flex-start' },
